  ...
  useEffect(() => {
    ...
    if (layer === LAYER.L1) {
      ...
      if (bridgeType === BRIDGE_TYPE.CLASSIC) {
        ...
        const lightBridgeExitFeeRaw =
          (100 - Number(l1LightBridgeFeeRateN)) / 100
        const value = Number(amount) * lightBridgeExitFeeRaw
        setLightBridgeExitFee(lightBridgeExitFeeRaw)
