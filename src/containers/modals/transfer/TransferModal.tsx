...
import { TransferModalInterface } from './types'

const TransferModal: React.FC<TransferModalInterface> = ({ open, token }) => {
  ...
  useEffect(() => {
    const estimateCost = async () => {
      let cost_BN: any = await networkService.transferEstimate(
        recipient,
      ...
      if (feeUseBoba) {
        cost_BN = cost_BN.mul(BigNumber.from(feePriceRatio))
      }
  ...
  const submit = async () => {
    if (token.address && recipient) {
      ...
      if (res) {
        dispatch(openAlert('Transfer submitted'))
      }
      handleClose()
    }
  }
      ...
      <Box>
          ...
          {!!recipient && (
            <Input
              ...
              onChange={(i) => {
                ...
                setValue_Wei_String(
                  toWei_String(i.target.value, token.decimals)
                )
              }}
              onUseMax={(i) => {
        ...
        {convertToUSD && (
          <Typography
            ...
            sx={{ opacity: 0.5, mt: 2 }}
          >
            {`Value in USD: $${amountToUsd(value, lookupPrice, token).toFixed(
              2
            )}`}
          </Typography>
        )}
        ...
        <Typography
          variant="body2"
          sx={{ mt: 2, fontWeight: '700', color: 'red' }}
        >
          CAUTION: This function is only for transfers from one Boba wallet to
          another Boba wallet. You cannot directly transfer funds from a Boba
          wallet to an L1 address or to another chain. Your funds will be lost
          if you try to do so.
        </Typography>
      </Box>
      <WrapperActionsModal>
        ...
        <Button
          ...
          tooltip={
            loading
              ...
              : 'Click here to transfer your funds to another Boba wallet'
          }
          disabled={!validValue}
