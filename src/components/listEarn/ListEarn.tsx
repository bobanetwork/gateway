...
class ListEarn extends React.Component<IListEarnProps, IListEarnState> {
  constructor(props: IListEarnProps) {
    super(props)
  ...
  async handleHarvest() {
    ...
    const userReward = BigNumber.from(userInfo.pendingReward)
      .add(
  ...
  render() {
    ...
    const formatNumber = (value: BigNumberish, limit?) => {
      ...
      },
      {
        content: (
          <Typography variant="body2">
            {' '}
            {userInfo.amount
              ? `${logAmount(userInfo.amount, decimals, 2)}`
              : `0`}
          </Typography>
        ),
        width: 90,
...
export default connect(mapStateToProps)(ListEarn as any)
